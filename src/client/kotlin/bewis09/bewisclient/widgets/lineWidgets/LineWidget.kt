 ...
 */
abstract class LineWidget<K>(id: String, protected val widget_width: Int, private val centered: Boolean) :
    Widget<K>(id) where K : SettingTypes.DefaultWidgetSettingsObject, K : SettingTypes.ColorTextSettingsObject {
        ...
        for ((index, text) in getText().iterator().withIndex()) {
            if (centered)
                drawContext.drawCenteredTextWithShadow(
                    ...
                    text,
                    x + getOriginalWidth() / 2,
                    y + 13 * index + 4,
                    (0xFF000000L + settings.getTextColor().getColor()).toInt()
                )
            if (!centered)
