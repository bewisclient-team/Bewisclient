...
class DaytimeWidget : LineWidget<SettingTypes.DaytimeWidgetSettingsObject>("daytime", 80, true) {
    override fun getText(): ArrayList<String> {
        ...
        val hour = NumberFormatter.zeroBefore(
            (if (!settings.clock24.get()) if (time.div(1000L) == 12L) time.div(1000L) else if (time.div(1000L) == 0L) 12 else time.div(1000L).rem(12L) else time.div(1000L)).toInt(), 2
        )
