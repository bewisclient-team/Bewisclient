...
class SnakeScreen : Screen(Text.empty()) {
    ...
    override fun render(context: DrawContext?, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        }, GREEN, (width - size) / 2 - size / 10, (height - size) / 2 - size / 10, 0f, 0f, size + size / 5, size + size / 5, size / 10, size / 10)
        RenderSystem.setShaderColor(1f, 1f, 1f, 1f)

        context!!.drawTexture({ texture: Identifier? ->
            RenderLayer.getGuiTexturedOverlay(texture)
        }, GREEN, (width - size) / 2, (height - size) / 2, 0f, 0f, size, size, size / 10, size / 10)
        ...
        SNAKE.forEach {
            ...
            context.drawTexture(
                if (i == SNAKE.size - 1) BLUE else RED,
