...
open class ScreenValuedAnimation(startValue: Float, endValue: Float) :
    ValuedAnimation(System.currentTimeMillis(), options_menu.animation_time.get().roundToLong(), EaseMode.EASE_IN_OUT, startValue, endValue) {}

open class ScreenValuedTypedAnimation(startValue: Float, endValue: Float, private val type: String) :
    ValuedAnimation(System.currentTimeMillis(), options_menu.animation_time.get().roundToLong(), EaseMode.EASE_IN_OUT, startValue, endValue) {
    fun getType(): String {
        return type
    }
