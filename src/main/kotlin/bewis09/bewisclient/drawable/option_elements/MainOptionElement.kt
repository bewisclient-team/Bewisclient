...
open class MainOptionElement: OptionElement, Search.SearchableElement<OptionElement> {
    ...
    override fun render(context: DrawContext, x: Int, y: Int, width: Int, mouseX: Int, mouseY: Int, alphaModifier: Long): Int {
        ...
        context.setShaderColor(1F,1F,1-animation.getValue()/6f, ((alphaModifier shr 24)*(if(outOfWorldDisabled && MinecraftClient.getInstance().world == null) 0.5f else 1f))/0xFF)
        context.drawTexture(image,x+6,y+6,32,32,0F,0F,32,32,32,32)
        context.setShaderColor(1F,1F,1-animation.getValue()/6f, if(outOfWorldDisabled && MinecraftClient.getInstance().world == null) 0.5f else 1f)
